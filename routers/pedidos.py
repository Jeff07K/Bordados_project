...
def create_pedido(
    ...
    pedido.usuario_email = current_user["sub"]
    session.add(pedido)
    session.commit()
...
def mis_pedidos(
    ...
    return session.exec(
        select(PedidoPersonalizado).where(
            PedidoPersonalizado.usuario_email == current_user["sub"]
        )
    ).all()
...
def cancel_pedido(
    ...
    if not p:
        raise HTTPException(404, "Pedido no encontrado")
    if p.usuario_email != current_user["sub"] and current_user.get("rol") != "admin":
